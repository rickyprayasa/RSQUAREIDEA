...
export async function POST(request: NextRequest) {
    try {
        console.log('Webhook ALT POST endpoint called');
        ...
        if (update.callback_query) {
            ...
            if (!confirmationId || !['approve', 'reject'].includes(action)) {
                console.log('Invalid confirmation ID or action:', { confirmationId, action });
                await answerCallbackQuery(
                    process.env.TELEGRAM_BOT_TOKEN || '',
                    callbackQuery.id,
                    'Invalid action',
...
export async function GET() {
    ...
    console.log('Telegram webhook ALT GET endpoint accessed');
    
    try {
        const config = await getTelegramConfig();
        return NextResponse.json({ 
            status: 'Telegram webhook ALT is active',
            config: {
