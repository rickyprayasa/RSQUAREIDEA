        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    className="fixed inset-0 z-[999999] bg-slate-950/80 sm:backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 md:p-6 overflow-hidden transform-gpu"
                    onClick={onClose}
                >
                    <motion.div
                        key="chatbot-modal-container"
                        initial={{ y: "100%" }}
                        animate={{ y: 0 }}
                        exit={{ y: "100%" }}
                        transition={{ ease: [0.32, 0.72, 0, 1], duration: 0.28 }}
                        onClick={(e) => e.stopPropagation()}
                        className="w-full max-w-3xl h-[92vh] sm:h-[88vh] max-h-[92vh] sm:max-h-[700px] min-h-[480px] bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col border-t sm:border border-gray-200 relative z-[1000000] transform-gpu will-change-transform"
                    >
                        ...
                        {!isCompleted && [3, 4, 5, 6, 7].includes(currentStep) && (
                            <div className="p-3 sm:p-4 bg-white border-t border-gray-100 z-20 flex-shrink-0">
                                <form onSubmit={handleTextSubmit} className="flex items-center gap-2 w-full">
                                    {currentStep === 7 ? (
                                        <textarea
                                            ...
                                            className="flex-1 min-w-0 bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:bg-white focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 resize-none transition-all"
                                        />
                                    ) : (
                                        <input
                                            ref={inputRef as any}
                                                ...
                                                currentStep === 3 ? 'Ketik nama lengkap Anda...' :
                                                    currentStep === 4 ? 'contoh: nama@gmail.com' :
                                                        currentStep === 5 ? '08xxxxxxxxxx (opsional)' :
                                                            currentStep === 6 ? 'Nama perusahaan (opsional)...' :
                                                                'Ketik balasan Anda...'
                                            }
                                            className="flex-1 min-w-0 bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:bg-white focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 transition-all"
                                        />
                                    )}
                                    ...
                                    {(currentStep === 5 || currentStep === 6) && (
                                        <button
                                            ...
                                            className="px-3 py-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs font-bold border border-gray-200 transition-all active:scale-95 flex-shrink-0 whitespace-nowrap"
                                        >
                                            Lewati
                                        </button>
                                    )}
                                    ...
                                    <button
                                        type="submit"
                                        disabled={isTyping}
                                        className="p-3 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-bold transition-all shadow-sm active:scale-95 disabled:opacity-50 flex-shrink-0 flex items-center justify-center min-w-[42px] h-[42px]"
                                    >
                                        <Send className="w-4 h-4" />
                                    </button>
                                </form>
